...
from stactools.deltares.commands import create_deltares_command


class CommandsTest(CliTestCase):
    def create_subcommand_functions(self) -> List[Callable[[Group], Command]]:
        return [create_deltares_command]
    ...
    def test_create_collection(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            ...
            result = self.run_command(f"deltares create-collection {destination}")
    ...
    def test_create_item(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            ...
            result = self.run_command(f"deltares create-item {infile} {destination}")
            self.assertEqual(result.exit_code, 0, msg="\n{}".format(result.output))
